...
interface Structs { //set up a struct called Val with a unsigned interger variable inside called value.
    struct Val {
        uint256 value;
    }

    enum ActionType { //enum works like structs but is meant to hold many different options. You can add enums inside of structs
    ...
    struct AssetAmount { // here we see that the enums above are added into this struct.
    ...
    struct ActionArgs { //here we see that the enum actionType is added inside of this struct.
